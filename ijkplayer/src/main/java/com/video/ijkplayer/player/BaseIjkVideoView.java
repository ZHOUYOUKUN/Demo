package com.video.ijkplayer.player;
...
import com.video.ijkplayer.bean.BaseVideoBean;
import com.video.ijkplayer.controller.BaseVideoController;
import com.video.ijkplayer.controller.MediaPlayerControl;
import com.video.ijkplayer.listener.OnVideoViewStateChangeListener;
import com.video.ijkplayer.listener.PlayerEventListener;
import com.video.ijkplayer.ui.StandardVideoController;
import com.video.ijkplayer.util.PlayerUtils;
import com.video.ijkplayer.util.ProgressUtil;
...
public abstract class BaseIjkVideoView extends FrameLayout implements MediaPlayerControl, PlayerEventListener {
    ...
    public boolean isHome = false; //是否首页视频
    ...
    protected void onOrientationLandscape(Activity activity) {
        if (mCurrentOrientation == LANDSCAPE) {
            return;
        }
    ...
    protected void onOrientationReverseLandscape(Activity activity) {
        if (mCurrentOrientation == REVERSE_LANDSCAPE) {
            return;
        }
        if (mCurrentOrientation == PORTRAIT
    ...
    public void setmMediaPlayer(AbstractPlayer mMediaPlayer) {
        ...
        setPlayState(BaseIjkVideoView.STATE_PLAYING);
        setPlayerState(BaseIjkVideoView.PLAYER_NORMAL);
...
//            mAudioFocusHelper.requestFocus();
    ...
    protected void startPrepare(boolean needReset) {
        ...
        if ((TextUtils.isEmpty(mCurrentUrl) && mAssetFileDescriptor == null) || mMediaPlayer == null) {
            return;
        }
        if (needReset) {
            mMediaPlayer.reset();
            setPlayState(STATE_RESET);
        }
        ...
        if (getVideoId() != null) {
            oncePlayId = getVideoId() + "_" + System.currentTimeMillis();
        }
    ...
    protected void startInPlaybackState() {
        if (mCurrentPlayState == STATE_PAUSED) {
            setPlayState(STATE_CANCEL_PAUSED);
        }
        mMediaPlayer.start();
    ...
    public void setNeedReset(boolean need) {
        isNeed = need;
    }
    ...
    public OnVideoViewStateChangeListener getmOnVideoViewStateChangeListeners() {
        if (mOnVideoViewStateChangeListeners != null && mOnVideoViewStateChangeListeners.size() > 0) {
            return mOnVideoViewStateChangeListeners.get(0);
        }
        return null;
    }
    ...
    public void onCompletion() {
        ...
        setKeepScreenOn(false);
        mCurrentPosition = 0;
        ProgressUtil.clearSavedProgressByUrl(mCurrentUrl);
    }
